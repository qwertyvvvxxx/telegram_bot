...
def get_quiz_after_action_keyboard(theme):
    """Генерує кнопки після відповіді, зберігаючи поточну тему"""
    return {
...
def get_gpt_service(context: ContextTypes.DEFAULT_TYPE):
    if 'gpt_service' not in context.user_data:
        context.user_data['gpt_service'] = ChatGptService(config.GPT_API_KEY)
...
async def gpt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    gpt = get_gpt_service(context)  # Отримуємо локальний сервіс
...
async def gpt_dialog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gpt = get_gpt_service(context)
    response_text = await gpt.send_question(prompt, user_text)
...
async def quiz_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. ОБРОБКА ВІДПОВІДІ (кнопки quiz_answer_X)
    if data.startswith("quiz_answer_"):
        ...
        # Отримуємо текст того, що вибрав користувач
        ...
        # Редагуємо старе повідомлення з питанням
        try:
            await query.edit_message_text(text=final_report)
        except Exception:
            pass

        # ПАУЗА, щоб користувач встиг прочитати результат
        await asyncio.sleep(3.5)

        # Надсилаємо меню наступних дій
    ...
    if data.startswith("quiz_"):
        ...
        gpt = get_gpt_service(context)
        gpt.message_list = []  # Обов'язково очищуємо пам'ять!
        ...
        # Функція очищення тексту варіантів від цифр на початку (1. Python -> Python)
